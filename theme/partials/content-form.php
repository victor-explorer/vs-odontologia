<section id="contato" class="wrapper text-center">
  <div class="container">
    <div class="row justify-content-xl-center">
      <div class="col-xl-7">
        <h4 class="mb-2 mb-xl-0">
          AGENDE SUA <em>CONSULTA</em>
        </h4>
        <p>
          Dê o primeiro passo rumo ao sorriso que sempre sonhou.
        </p>

        <div class="vs-form mt-xl-5">
          <?php echo do_shortcode('[contact-form-7 id="8c21a00" title="Formulário de contato"]'); ?>
        </div>

        <div class="mt-5">
          <a href="https://wa.me/5511914155003" class="vs-flex-topic" aria-label="Chat no WhatsApp" target="_blank">
            <img src="<?php echo get_template_directory_uri() . '/assets/images/icons/whatsapp.svg'; ?>" alt="" width="46" height="46" class="img-fluid">
            +55 11 91415 5003
          </a>
        </div>
      </div>
    </div>

  </div>
</section>